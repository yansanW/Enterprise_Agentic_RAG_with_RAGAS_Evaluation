...
if uploaded_file and COHERE_API_KEY :
    ...
    if st.session_state.get("last_file") != uploaded_file.name or st.session_state.qa_chain is None:
        ...
        # It uses your embedding model to calculate distance mathematically!
        # You can set the breakpoint threshold type to 'percentile', 'standard_deviation', or 'interquartile'
        ...
        embeddings = OllamaEmbeddings(model="nomic-embed-text")  # best local embed model
        ...
        llm = ChatOllama(model="llama3")        # or mistral, gemma2, etc.
