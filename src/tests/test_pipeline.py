...
def mock_pipeline(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ContextualCompressionRetriever, "_get_relevant_documents", mock_compress_async)
    monkeypatch.setattr(ContextualCompressionRetriever, "_aget_relevant_documents", mock_compress_async)
    # ---------------------------------------
...
async def test_router_selects_retrieve_for_factual_queries(mock_pipeline):
    decision = await mock_pipeline.aroute_query("Who founded Alpha Corp and in what year?")
