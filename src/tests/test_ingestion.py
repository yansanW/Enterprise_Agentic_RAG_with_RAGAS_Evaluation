...
def test_pdf_processing_handles_nonexistent_file(monkeypatch):
    ...
    monkeypatch.setattr(
        config, "PARSER_STRATEGY", "standard_text"
    )  # Ensure it routes to the standard text parser for this test
...
def test_pdf_processing_outputs_correct_document_structure():
    ...
    if not os.path.exists(test_file_path):
        pytest.skip(
            "Local test PDF not present, skipping structural matrix verification."
        )
